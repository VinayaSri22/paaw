...
class Agent:
    ...
    async def _load_tools_schema(self):
        ...
        # In lite mode chat doesn't use MCP tools, so don't spawn MCP server
        # containers just to list their schemas (they would linger). Jobs load
        # and tear down their own MCP servers via the executor.
        from paaw.config import settings
        if settings.lite_mode:
            logger.info("Lite mode: skipping MCP tool loading for chat")
            return
